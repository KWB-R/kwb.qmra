---
title: "Usage (Loguniform Inflow)"
author: "Michael Rustler"
date: "`r format(Sys.time(), '%d %B, %Y')`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Technical Workflow: Usage (Loguniform Inflow)}
...
The folder with the csv configuration files  for a hypothetical use case is 
located here: 

```{r}
#### DEFINE DIRECTORY ################
confDir <- system.file("extdata/configs/dummy_loguniform", package = "kwb.qmra")
confDir
```

The following screenshot shows the required configuration files located in 
`confDir` (here: `r confDir`):
...
## 5.1 Inflow
```{r, fig.width = 12,  fig.cap="Simulated inflow concentrations", warning= FALSE}
kwb.qmra::plot_inflow(risk)
```

## 5.2 Treatment
```{r, fig.width = 12, fig.cap="Simulated reductions in the treatment plant", warning= FALSE}
kwb.qmra::plot_reduction(risk)
```

## 5.3 Effluent
```{r, fig.width = 12, fig.cap="Simulated effluent concentrations", warning= FALSE}
kwb.qmra::plot_effluent(risk)
```

## 5.4 Exposure 
```{r, fig.width = 12, fig.cap="Simulated dose per event", warning= FALSE}
...
### 5.5.1 Per event
```{r, fig.width = 12, fig.cap="Simulated infection probability per event", warning= FALSE}
kwb.qmra::plot_event_infectionProb(risk)
```

```{r, fig.width = 12, fig.cap="Simulated illness probability per event", warning= FALSE}
kwb.qmra::plot_event_illnessProb(risk)
```
```{r, fig.width = 12, fig.cap="Simulated DALYs per event", warning= FALSE}
kwb.qmra::plot_event_dalys(risk)
```

### 5.5.2 Total 

```{r, fig.width = 12, fig.cap="Simulated total infection probability (for all events)", warning= FALSE}
kwb.qmra::plot_total_infectionProb(risk)
```

```{r, fig.width = 12, fig.cap="Simulated total illness probability (for all events", warning= FALSE}
kwb.qmra::plot_total_illnessProb(risk)
```

```{r, fig.width = 12, fig.cap="Simulated total DALYs (for all events)", warning= FALSE}
kwb.qmra::plot_total_dalys(risk)
```

In addtion tables with summary statistics, e.g. for the **total risk** can be generated easily 
as shown below:
...
E.g. Write reports for all configurations in package subfolder `extdata/configs/` (here: 
`r system.file("extdata/configs/", package = "kwb.qmra")`):
