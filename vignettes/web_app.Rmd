...
library(kwb.qmra)
data(config_json)
writeLines(text = config_json, "config.json")
```
...
in the example data [config.json](../config.json) (for details see: 
`kwb.qmra::config_json`)."
)
```
...
for `kwb.qmra::opencpu_simulate_risk()` 
required in a JSON data structure as provided 
in the example data [config_json](config_json) (for details see: 
`kwb.qmra::config_json`)."
)
```
...
### Optionally directly import configurations from CSVs
